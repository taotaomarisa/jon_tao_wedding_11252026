...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; padding: 20px; color: #1f2937;">
  ...
  <table style="width: 100%; border-collapse: collapse; margin-top: 24px;">
    ...
    <tr><td style="padding: 10px 0; font-weight: 600;">Contact Email</td><td style="padding: 10px 0;">${data.guestEmail}</td></tr>
...
export async function POST(request: NextRequest) {
  ...
  const missingGuestSelections = submittedGuests.some(
    (guest) =>
      guest.rsvp === 'Attend' &&
      (!guest.starter || !guest.main || !guest.dessert || !guest.activity),
  );
